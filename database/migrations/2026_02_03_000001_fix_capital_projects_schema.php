...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // Drop and recreate the enum with correct values (Postgres-only DDL;
        // SQLite has no CHECK-constraint rename/type-change support here, and
        // its columns are already plain text, so skip on non-pgsql drivers).
    ...
    public function down(): void
    {
        ...
        // Restore original enum constraint (Postgres-only DDL)
        if (DB::getDriverName() === 'pgsql') {
            DB::statement('ALTER TABLE capital_projects DROP CONSTRAINT IF EXISTS capital_projects_status_check');
