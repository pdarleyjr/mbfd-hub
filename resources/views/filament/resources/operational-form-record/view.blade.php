<x-filament-panels::page>
    @php
        ...
        $formLabel = match ($this->record->form_type) {
            'ics_214' => 'ICS 214',
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm xl:col-span-1">
            ...
            <dl class="mt-6 space-y-4 text-sm">
                ...
                <div><dt class="text-gray-500">Document</dt><dd class="font-medium text-gray-900">{{ $latestDocument ? $latestDocument->display_name : 'No document' }}</dd></div>
            </dl>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm xl:col-span-2">
            @if ($latestDocument)
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div><h2 class="text-lg font-semibold text-gray-950">Authenticated document</h2><p class="text-sm text-gray-500">{{ $latestDocument->display_name }} · SHA-256 {{ Str::limit($latestDocument->pdf_sha256, 20) }}</p></div>
                ...
                @if ($latestDocument->isInlinePreviewable())
                ...
                @else
                    <div class="grid min-h-80 place-items-center rounded-lg border border-dashed border-gray-300 bg-gray-50 p-8 text-center text-gray-600">
                        <div><x-heroicon-o-document-arrow-down class="mx-auto h-10 w-10"/><p class="mt-3 font-medium">This file type opens in its installed application.</p><p class="mt-1 text-sm">Use Download to view the original file safely.</p></div>
                    </div>
                @endif
            @else
    ...
    <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-gray-950">Document history</h2>
        <div class="mt-4 overflow-x-auto"><table class="w-full text-left text-sm"><thead class="border-b border-gray-200 bg-gray-50 text-xs uppercase text-gray-500"><tr><th class="px-3 py-2">Version</th><th class="px-3 py-2">Source revision</th><th class="px-3 py-2">Submitted / generated</th><th class="px-3 py-2">Checksum</th><th class="px-3 py-2">Actions</th></tr></thead><tbody class="divide-y divide-gray-100">@forelse ($this->record->documents->sortByDesc('version_number') as $document)<tr><td class="px-3 py-3 font-medium">{{ $document->version_number }}</td><td class="px-3 py-3">{{ $document->source_revision }}</td><td class="px-3 py-3">{{ $document->created_at?->format('M j, Y g:i A') }}</td><td class="px-3 py-3 font-mono text-xs">{{ Str::limit($document->pdf_sha256, 18) }}</td><td class="px-3 py-3"><div class="flex flex-wrap items-center gap-2"><a class="font-semibold text-primary-600 hover:underline" href="{{ route('admin.operational-forms.documents.preview', $document) }}" target="_blank">Open</a><a class="font-semibold text-primary-600 hover:underline" href="{{ route('admin.operational-forms.documents.download', $document) }}">Download</a>@if ($this->record->form_type !== 'uploaded_file')<form method="POST" action="{{ route('admin.operational-forms.documents.destroy', ['document' => $document, 'redirect' => 1]) }}" onsubmit="return confirm('Permanently delete this PDF version? This cannot be undone.')">@csrf @method('DELETE')<button type="submit" class="font-semibold text-danger-600 hover:underline">Delete PDF</button></form>@endif</div></td></tr>@empty<tr><td colspan="5" class="px-3 py-8 text-center text-gray-500">No documents available.</td></tr>@endforelse</tbody></table></div>
    </section>
</x-filament-panels::page>
