@php
    ...
    $fallback = $returnTarget ?? \App\Filament\Employee\Pages\EmployeeDashboard::getUrl(panel: 'employee');
@endphp
<div class="employee-global-back">
    <a
        href="{{ $fallback }}"
        @if(!$returnTarget) onclick="if (window.history.length > 1) { event.preventDefault(); window.history.back(); }" @endif
        aria-label="Go back"
    >
        ...
        <span>{{ $returnTarget ? 'Back to Station' : 'Back' }}</span>
    </a>
