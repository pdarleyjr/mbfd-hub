...
namespace App\Services\MediaControl;
...
final class MediaControlAuthorizationCodeBroker
{
    private const CACHE_PREFIX = 'media-control:authorization-code:';

    public function issue(User $user, string $clientId, string $redirectUri): string
    {
        $code = rtrim(strtr(base64_encode(random_bytes(32)), '+/', '-_'), '=');
        $ttl = (int) config('services.media_control.authorization.code_ttl_seconds', 60);
        $now = now()->getTimestamp();

        $stored = Cache::put($this->cacheKey($code), [
            'issuer' => (string) config('services.media_control.authorization.issuer'),
            'audience' => $clientId,
        ...
        if (! $stored) {
            throw new RuntimeException('Media Control authorization code storage is unavailable.');
        }
    ...
    public function redeem(string $code, string $clientId, string $redirectUri): ?array
    {
        ...
        try {
            return Cache::lock($key.':redeem', 5)->block(1, function () use ($key, $clientId, $redirectUri): ?array {
    ...
    private function isValidRecord(mixed $record, string $clientId, string $redirectUri): bool
    {
        ...
        return ($record['issuer'] ?? null) === (string) config('services.media_control.authorization.issuer')
