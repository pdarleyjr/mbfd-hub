...
class EvaluationService
{
    ...
    public function getCategoryRankings(int $categoryId, ?int $sessionId = null): Collection
    {
        ...
        if ($sessionId) $query->where('workgroup_session_id', $sessionId);

        return $query->get()->map(function ($product) {
            $scores = $product->submissions->map(fn($s) => $s->overall_score)->filter(fn($s) => $s !== null);
            return [
                'product' => $product,
                'weighted_average' => $scores->isNotEmpty() ? round($scores->avg(), 2) : null,
                'response_count' => $product->submissions->count(),
                'meets_threshold' => $product->submissions->count() >= $this->minimumResponseThreshold,
            ];
        })->sortByDesc(fn($item) => [$item['weighted_average'] ?? -1, $item['response_count']])->values();
    }

    public function getTopProducts(int $categoryId, ?int $sessionId = null, int $limit = 2): Collection
    {
        return $this->getCategoryRankings($categoryId, $sessionId)->filter(fn($item) => $item['meets_threshold'])->take($limit);
    }

    public function getFinalists(?int $sessionId = null): Collection
    {
        $finalists = collect();
        foreach (EvaluationCategory::rankable()->active()->get() as $category) {
            foreach ($this->getTopProducts($category->id, $sessionId, 2) as $index => $item) {
                $finalists->push([
                    'category' => $category,
    ...
    {
        $feedback = collect();
        foreach (EvaluationCategory::where('is_rankable', false)->active()->get() as $category) {
            $submissions = EvaluationSubmission::whereHas('candidateProduct', function ($q) use ($category, $sessionId) {
                $q->where('category_id', $category->id);
                if ($sessionId) $q->where('workgroup_session_id', $sessionId);
            ...
            if ($submissions->isNotEmpty()) {
                $feedback->push([
                    ...
                    'feedback' => $submissions->map(fn($s) => [
                        'submission_id' => $s->id,
                        'evaluator' => $s->member?->user?->name,
                        'product' => $s->candidateProduct->display_name ?? $s->candidateProduct->name,
                        'score' => $s->overall_score,
                        'comments' => $s->comments->pluck('comment', 'category_id')->toArray(),
    ...
    public function getSessionResults(?int $sessionId = null): array
    {
        ...
        foreach (EvaluationCategory::rankable()->active()->get() as $category) {
            ...
            $categoryRankings[] = [
                ...
                'is_rankable' => true,
                'rankings' => $rankings,
                'top_products' => $rankings->filter(fn($i) => $i['meets_threshold'])->take(2)->values(),
                'total_products' => $rankings->count(),
                'eligible_products' => $rankings->filter(fn($i) => $i['meets_threshold'])->count(),
            ];
        }
        return [
    ...
    public function getSessionProgress(?int $sessionId = null): array
    {
        ...
        $subQuery = EvaluationSubmission::whereHas('candidateProduct', fn($q) => $sessionId ? $q->where('workgroup_session_id', $sessionId) : $q);
        $totalSubmissions = (clone $subQuery)->count();
        $draftSubmissions = (clone $subQuery)->where('status', 'draft')->count();
        $submittedSubmissions = (clone $subQuery)->where('status', 'submitted')->count();
        return [
    ...
    {
        $evaluatedProductIds = EvaluationSubmission::where('workgroup_member_id', $member->id)->pluck('candidate_product_id')->toArray();
    ...
    /**
     * Get or create a draft submission. If a submitted (non-locked) submission exists, return it for editing.
     */
    public function getOrCreateDraft(WorkgroupMember $member, int $productId): EvaluationSubmission
    {
        // Find ANY existing submission (draft or submitted)
    ...
    public function getProductStats(int $productId): array
    {
        ...
        $submissions = EvaluationSubmission::where('candidate_product_id', $productId)->where('status', 'submitted')->get();
