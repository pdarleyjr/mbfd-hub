...
class ApparatusSheetSyncService
{
     ...
     *
     * Priority logic:
     *  - If current_location is set and differs from assignment, show "Assignment → current_location"
     *  - If assignment equals current_location (or current_location empty), show the station display
     *  - Station display: "Station {station_number}" if station present, else assignment, else current_location
     */
    private function buildLocation(Apparatus $a): string
    {
        ...
        // Normalise — treat identical strings as one
        if ($currentLoc && $currentLoc === $assignment) {
            $currentLoc = '';
        }

        // If apparatus is deployed away from assignment, show both
        if ($currentLoc && $assignment && $currentLoc !== $stationLabel) {
            return "{$assignment} → {$currentLoc}";
        }

        // Use current_location if set, otherwise assignment, otherwise station label
        return $currentLoc ?: $assignment ?: $stationLabel ?: '';
    }

    /**
     * Bootstrap the Google API client using service account credentials.
     */
    ...
    /**
     * Read spreadsheet metadata and verify the target tab exists with the
     * expected title and sheetId. Fail closed on mismatch.
     */
    ...
    /**
     * Execute a callable with truncated exponential backoff.
     * Retries on Google_Service_Exception with 429/500/503 status codes.
     */
    private function withRetry(callable $callback): mixed
    {
        ...
        while (true) {
            try {
                ...
                // Truncated exponential backoff: base * 2^(attempt-1) + jitter, max 32s
