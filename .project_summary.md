...
- **Admin panel**: `super_admin`, `admin`, `training_admin`, or `training_viewer` role (includes LOGIN_SCREEN_SCOPE)
...
| Role | Count | Users |
|------|-------|-------|
| `super_admin` | 13 | All logistics + Peter Darley |
| `admin` | 13 | Same users (dual-roled) |
| `training_admin` | 5 | All Training users |
| `training_viewer` | 0 | None assigned |

**Employee users** (go-to `/training` instead of `/admin`):
...
**Logistics admin users** (go-to `/admin`):
...
- All admin users include LOGIN_SCREEN_SCOPE for `/` redirect

#### Good Architecture Pattern

**The `station_name` column exists and is used correctly in existing code**.

Examples:
```php
QrCode::size(100)
    ->generate($station['station_number'])

サラカラーでは CONST_STATION の個件検索機能で 20-219 記の中に入れられています。
  
    $memberService.exceptions["exception"] =      # Handle restrict exception case
        VIPFlagPlus::new(0, 0);
      break;
  
    // 専用フラグが付いているか調べる
    $triesHashMember = hash::check($memberwifiid, $row["ssid"]);
    if ($triesHashMember) {
      // もう一度登録しなおうか(PPP flag)聞くGET
      $mayexcept = file_get_contents("$membershopper/?try={$row["try"]}");
      $closeflag = json_decode($mayexcept, true);
      switch ($closeflag["flag"]) {
        case "open":
          $memberService.exceptions["flag"] = "open";
          break;
        case "paiza":
          $memberService.exceptions["flag"] = "paiza";
          break;
        case "vip":
          $memberService.exceptions["flag"] = "vip";
          break;
        case "wifialogin":        # Unknown flag, use logon
          $memberService.exceptions["flag"] = "wifialogin";
          break;
        default:
          break;
      }
    } else {
      $memberService.exceptions["email"] or $memberService.exceptions["tel"] or
      $memberService.exceptions["type"] or $memberService.exceptions["plan"] or
      $memberService.exceptions["exception"] or flowAction::stop();
    }
  } else {
    // もう一度登録しなおうか(PPP flag)聞くGET
    $mayexcept = file_get_contents("$membershopper/?try={$row["try"]}");
...
if ($memberService.exceptions["flag"] ?? false) {
  $cnfFormat = new cfncsvformat::new();

  [$cpppService] = GetCoflucppp(
    1,
    $config["pppuser"],
    $config["ppppass"]
  );

  $control = $cpppService->control();
  $confuser[$control["user"]["name"]]  = $user_displayname;

  // 実際の PPP を止めてはいけないテスト
  // const bool testMode = true;
  // if(!$testMode) {
  $confuser = json_encode($confuser);
  $disabled = ($memberService.exceptions["flag"] === "open") ? "" : "disabled";

  $transport = $cnfFormat->encode(array("from" => 3,
         "param" => array("action" => "ADPolicy_Update", //CSRS-572 ppp custom policy of MenuReservation PAGE
                  "type" => "policy",       //CSRS-572 ppp custom policy of MenuReservation PAGE
                  "actionp1" => "ADPolicy_Update",
                  "paramp1" => array("action" => "SSH",
                  "param" => json_encode(array(
                      ...
                      "conf" => array(
                          "confuser" => $confuser,
                          "cmd" => array(
                            "ui-policy-ADPolicy.json",   //CSRS-572
                      ),
                   "mode" => "3",
                 ),
                 ),
  ...
  // CSRS-572 予約サービスメニュー予約確認画面取得
  /* @var $aa libAjaxAction */
  $aa = plugin_action("admin_action::view::menu_reservation_confirmation");

  //echo '<br/>getXSDStr ( $_SERVER["REMOTE_ADDR"] ) ='.getXSDStr( $_SERVER["REMOTE_ADDR"] );
  //echo '<br/> $control["remark"]["flag"] = '.$control["remark"]["flag"];
  //echo '<br/> $getsFormat["RESERVE"]["flag"] = '.$getsFormat["RESERVE"]["flag"];

  $receiveID = $pppuser;
  $timeShift = $TimeZoneManager->getShiftdiff($aUser);
  $jsdate = date($GLOBALS["DATE_FORMAT_JS"], time() + $timeShift);

  outputbox::start(
    "js_return::ok",
    $_SERVER["REMOTE_ADDR"],
    "FFT到着予約システム<br/>
    ネットワークサービスメニュー予約確認画面",
    "text/information"
  );

  echo $aa->flash($transport)
    ->set("token", $token)
    ->set("showmsg", 0)
    ->set("device_type", $device_type)
    ->set("timezone", json_encode($php_time))
    ->set("list:All_Time", transpose_can(dimensions,$device_set["All_Time"]) )
    ->set("list:STTS", transpose_can(dimensions,$device_set["STTS"] ))
    ->set("list:SYST", transpose_can(dimensions,$device_set["SYST"] ))
    ->set("list:PDTS", transpose_can(dimensions,$device_set["PDTS"] ))
    ->set("list:user01touch", transpose_can(dimensions,$device_set["user01touch"] ))
    ->set("fix", json_encode(array("col0" => $org_x, "col1" => $org_y)) )
    ->set("plusmstranspose.dimensionsifier", $transpose_dimensions)
    ->set("runtime", $jsdate)
    ->set("action", "test")
    ->getPublicJSON();
  die();
...
### nojson...
echo '<br/>ppphelper on start<br/>';

// 2022-07-15 tinyurl の判断 ⇒デフォはしないや manifests を 転載するようにしたい
// 非 tinyurl の内メニューは空白枠 but tinyurl の場合はデフォみついて空白枠 erased
// 最初のチェックスイッチ在 Succ plant のhung channel check(f=touch)
/**
  Def Input box ---
  devices_ref( STTS ＆All)として参照するdirection一行方向のチェック項目(STTSできなければ立ち上火锅費投入しないようにする)
  devices_ref で df_otsu による potouch が渡って来る予約コード
  隠れフィールド potouch で Hung_Succ を加えてみるよう改作
    ---
  転載するtinyurl の場合は別に対応するだけで原來のJSは使える
  tiny url が接続でき Yapı横がしない js_forward を呼び出して 空白枠 erased してページを付けauとするでDEfault touchがON
    ---
  非tinyurl で 隣端 igual無し hous hottest 実機なので DEFを停覚default touch
    ---
*/

transplsentenceNojsoniok:

if($org_x == 0 and $org_y == 0 and $device_type != "tinyurl"){
  //echo '<br/>Nojson rule #1.亲身との混雑感を防ぐためAll_Time STTS SYST全てフル送れない!<br/>';
}

if($poster == '1'){
  //echo '<br/> def rule stts syystem are not touch (1: action rule,got success potouch checkikiplus ok)<br/>';
  $device_type = "def";
}

if('minishift-' == $org_x or 'shift-' == $org_y){
  //echo '<br/> def rule stts syystem are not touch (2:full action rule,got success potouch checkikiplus ok)<br/>';
  $device_type = "def";
}

if('show/' == $org_x or 'show/' == $org_y){
  //echo '<br/>minishift成功呈もたらな~~~~ajax直実行!<br/>';
}
