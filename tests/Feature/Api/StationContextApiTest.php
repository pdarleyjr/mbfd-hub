...
class StationContextApiTest extends TestCase
{
    ...
    public function test_station_activity_includes_station_requests_but_never_personal_employee_requests(): void
    {
        ...
        $otherRequest = $this->stationRequest($otherStation, $employee);
        ...
        $response = $this->getJson("/api/public/stations/{$station->id}/activity")
            ...
            ->assertJsonPath('activity.0.status', 'under_review');
        ...
        $this->assertStringNotContainsString($otherRequest->request_number, $response->getContent());
    }
    ...
    public function test_room_profile_keeps_retired_asset_events_and_open_requests_older_than_history_limit(): void
    {
        $station = $this->station();
        $employee = $this->employee();
        $room = Room::query()->create([
        ...
        $retiredAsset = RoomAsset::query()->create([
            'room_id' => $room->id,
            'name' => 'Retired television',
            'quantity' => 1,
            'condition' => 'retired',
            'is_active' => false,
            'retired_at' => now(),
        ]);
        $oldOpen = $this->stationRequest($station, $employee, $room);
        $oldOpen->timestamps = false;
        $oldOpen->forceFill(['created_at' => now()->subYear(), 'updated_at' => now()->subYear()])->saveQuietly();
        $oldOpen->timestamps = true;
        foreach (range(1, 51) as $index) {
            $closed = $this->stationRequest($station, $employee, $room, 'completed');
            $closed->forceFill(['created_at' => now()->subMinutes($index)])->saveQuietly();
        }
        RoomAssetEvent::query()->create([
            'room_asset_id' => $retiredAsset->id,
            'event_type' => 'retired',
            'event_at' => now(),
        ]);
        $this->actingAs($employee, 'employee');
        ...
        $this->assertContains($oldOpen->request_number, collect($response->json('open_requests'))->pluck('request_number'));
        $this->assertContains('retired', collect($response->json('asset_events'))->pluck('event_type'));
        $this->assertNotContains('Retired television', collect($response->json('current_assets'))->pluck('name'));
        $this->assertCount(50, $response->json('request_history'));
    }
