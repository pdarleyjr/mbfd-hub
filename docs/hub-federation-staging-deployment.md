...
Create `/opt/mbfd/staging/hub-federation/runtime.env` from
`docker/staging/runtime.env.example` with mode `0600`. Generate a unique
`APP_KEY`, database password, Cloudflare staging-tunnel token, Bid reader token,
and three staging-only test passwords. Do not copy any production secret.
...
```bash
HUB_STAGING_RUNTIME_ENV=/opt/mbfd/staging/hub-federation/runtime.env \
