...
| P1 | Hosted CI partially observed / release gate blocked | Named draft-PR hosted jobs now provide CI evidence, but no all-green current-SHA gate exists because Pint fails and GitHub marked the secret-scan job cancelled after Gitleaks completed. Staging/candidate deployment, production endpoint/browser, notification/queue, backup/rollback, and Command Display release evidence remain unobserved. |
