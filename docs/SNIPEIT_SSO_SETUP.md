...
### ✅ Completed
- **5 users created in Snipe-IT** with superuser permissions (initial password set out-of-band)
...
- **Password**: out-of-band (not in this repo)
...
All users have a temporary password (set out-of-band; not in this repo) that must be changed on first login.
